...
def recovery_overlap(
    ...
    # Ground truth: GO/BioGRID (kept as in your code)
    ...
    # Preprocess SAINT data to include missing preys (unchanged)
    ...
    # --- NEW: limma handling (minimal) ---
    # Ensure prey column name and add missing preys with adj.P.Val=1.0
    limma_df_raw = limma_df_raw.rename(columns={'Protein': 'Prey'})  # if already Prey, harmless
    ...
    limma_df['limma_score'] = 1.0 - limma_df['adj.P.Val']  # score = 1 - adj.P.Val

    # Clean method-specific dataframes (unchanged for others)
    ...
    df_li = prepare_method_df(limma_df,         'Bait', 'Prey',     'limma_score', 'limma')   # <-- NEW

    # Normalize composite score globally (unchanged)
    ...
    # Combine all methods (now incl. limma)
    df_all = pd.concat([df_pu, df_q, df_ex, df_li, df_comp], ignore_index=True)

    # Evaluate overlap across top N interactions (unchanged)
    step = 10
    max_n = 2000
    ...
    # --- Add individual features (unchanged) ---
    individual_features = [
